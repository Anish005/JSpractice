...
if (playGame) {
  submit.addEventListener('click', function (e) {
    ...
    //console.log(guess) check if number is there or not
    validateGuess(guess);
  });
}

function validateGuess(guess) {
  //if the input is in the correct order as per the game
  if (isNaN(guess)) {
...
function displayGuess(guess) {
  //clean values for the next input and update the prevGuess array
  userInput.value = '';
...
const stopChangingColor = function () {
  ...
  intervalID = null; // flushing out the variable if not in use
};
